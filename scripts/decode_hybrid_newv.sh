...
#    $model $graphdir/HCLG.fst ark:- "ark,t:|gzip -c > $dir/lat.JOB.gz" || exit 1;
#fi
...
#	nnet-forward --feature-transform=$feature_transform --no-softmax=$no_softmax --apply-log=$apply_log --class-frame-counts=$class_frame_counts --use-gpu-id=$use_gpu_id $nnet "$feats" ark:$dir/$JOB.post 2> $dir/log/pre.$JOB.log || exit 1 &
	nnet-forward --feature-transform=$feature_transform --no-softmax=$no_softmax --apply-log=$apply_log --prior-scale=$prior_scale --class-frame-counts=$class_frame_counts --use-gpu=$use_gpu_id $nnet "$feats" ark:- | \
	decode-faster-mapped --max-active=$max_active --beam=$beam --acoustic-scale=$acwt --allow-partial=true --word-symbol-table=$graphdir/words.txt $model $graphdir/HCLG.fst ark:- ark,t:$dir/test.$JOB.tra ark,t:$dir/test.$JOB.ali 2> $dir/log/decode.$JOB.log || exit 1 &
	sleep 5
done
...
#for JOB in $(seq $nj);
#do
#	decode-faster-mapped --max-active=$max_active --beam=$beam --acoustic-scale=$acwt --allow-partial=true --word-symbol-table=$graphdir/words.txt $model $graphdir/HCLG.fst ark:$dir/$JOB.post ark,t:$dir/test.$JOB.tra 2> $dir/log/decode.$JOB.log || exit 1 &
#done

...
tools=/project/bdda/skhu/Download/kaldi-trunk-new/egs/uaspeech/s5
...
scheduler=train_nnet_scheduler.sh
if [ "$newbob" == "true" ]; then
	scheduler=train_nnet_scheduler_newbob.sh
fi

bash ${tools}/${scheduler} \
  --feature-transform $feature_transform \
  --seed $seed \
  ${objective_function:+ --objective-function "$objective_function"} \
  ${learn_rate:+ --learn-rate "$learn_rate"} \
  ${learn_rate_dec_start:+ --learn-rate-dec-start "$learn_rate_dec_start"} \
  ${train_opts} \
